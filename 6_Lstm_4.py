...
with graph.as_default():
    ...
    sumx = tf.concat(1, [ix, fx, cx, ox])
    summ = tf.concat(1, [im, fm, cm, om])
    sumb = tf.concat(1, [ib, fb, cb, ob])
    # Variables saving state across unrollings.
    saved_output = tf.Variable(tf.zeros([batch_size, num_nodes]), trainable=False)
    saved_state = tf.Variable(tf.zeros([batch_size, num_nodes]), trainable=False)
    # Classifier weights and biases.
    ...
    def lstm_cell(i, o, state):
        ...
        Note that in this formulation, we omit the various connections between the
        previous state and the gates."""
        y = tf.matmul(i, sumx) + tf.matmul(o, summ) + sumb
...
# 
# This has not been tested because test takes a while and I can't wait that long because I have other tasks to do.
# ---
...
class BatchGeneratorSeq2Seq(object):
    def __init__(self, text, batch_size, num_unrollings):
        self._text = text
        self._text_size = len(text)
        self._batch_size = batch_size
...
train_batches = BatchGeneratorSeq2Seq(train_text, batch_size, num_unrollings)
valid_batches = BatchGeneratorSeq2Seq(valid_text, 1, num_unrollings)

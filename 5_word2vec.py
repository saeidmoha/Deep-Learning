...
# I added some comments by myself
# details: https://www.tensorflow.org/tutorials/word2vec
#%matplotlib inline
...
print("dictionary= ", dictionary.items()[:10])
print("reverse_dictionary= ", reverse_dictionary.items()[:10])
...
with graph.as_default(), tf.device('/cpu:0'):
  ...
  # The skip-gram model takes two inputs. One is a batch full of integers representing the source context words, 
  # the other is for the target words. Let's create placeholder nodes for these inputs, so that we can feed in data later.
  # Input data.
...
with tf.Session(graph=graph) as session:
  ...
  for step in range(num_steps):
    ...
    # Training the model is then as simple as using a feed_dict to push data into the placeholders
    # and calling tf.Session.run with this new data in a loop.
